...
```{r project.setup, message=FALSE, warning=FALSE, results='hide', echo = TRUE}
#Import libraries
...
```{r echo=FALSE, fig.align='left', fig.cap="boxplot of ozone levels by month, 2018", fig.height=2.8}
#Create boxplot of ozone levels by month for 2018 records
epa_data %>% 
  filter(Year == 2018) %>% 
  drop_na(Ozone) %>% 
...
```{r echo=FALSE, fig.align='left', fig.cap="boxplot of ozone levels by month, 2019", fig.height=2.9}
#Create boxplot of ozone levels by month for 2019 records
epa_data %>% 
  filter(Year == 2019) %>% 
  drop_na(Ozone) %>% 
  mutate(Month = factor(Month)) %>%
...
[https://github.com/Tonysflex/EDA-Spring2023](https://github.com/Tonysflex/EDA-Spring2023)
